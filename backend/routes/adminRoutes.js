...
/*DASHBOARD*/
router.get("/orders-for-upcoming-shifts", authenticate, requireRole("admin"), 
getOrdersForUpcomingShifts);
router.get("/orders-for-shift", authenticate, requireRole("admin"), 
getOrdersForShift);
router.get("/orders-with-summary-for-shift", authenticate, requireRole("admin"), 
getOrdersWithSummaryForShift);

/*JOB APPLICATION AND USER MANAGEMENT*/ 
router.put("/updateApplication/:uid", authenticate, requireRole("admin"), updateApplicationStatus); // Route restricted to admin for approving employee accounts
...
router.get("/getApplications", authenticate, requireRole("admin"),getAllApplications); // get all employment applications
