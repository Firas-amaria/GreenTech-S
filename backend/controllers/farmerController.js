...
async function updateFarmerLands(farmerId, lands) {
  ...
  console.log(
    `🔧 DEBUG: Cleaned lands for Firebase:`,
    JSON.stringify(cleanedLands, null, 2)
  );
...
async function getFrontendLands(req, res) {
  try {
    ...
    console.log(
      `[getFrontendLands] Returning ${transformedLands.length} transformed lands`
    );
...
async function createCrop(req, res) {
  try {
    ...
    console.log(
      `[createCrop] Creating crop for farmerId: ${farmerId}, farmId: ${farmId}`
    );
    ...
    if (!isNaN(farmId)) {
      ...
      if (index >= 0 && index < lands.length) {
        landIndex = index;
        console.log(
          `🔧 DEBUG: Converted farmId ${farmId} to landIndex: ${landIndex}`
        );
      }
    }
    ...
    console.log(
      `🔧 DEBUG: Using landIndex: ${landIndex}, landName: ${lands[landIndex].landName}`
    );
    ...
    console.log(
      `🔧 DEBUG: Updated land object:`,
      JSON.stringify(lands[landIndex], null, 2)
    );
...
async function manageInventory(farmerId, crop, landIndex, action) {
  try {
    ...
    console.log(
      `🔧 DEBUG: manageInventory called with action: ${action}, cropId: ${cropId}`
    );
    ...
    if (action === "add") {
      ...
      if (docSnapshot.exists) {
        ...
        console.log(
          `⚠️ Crop ${cropId} not found in inventory, nothing to remove`
        );
      }
    }
  } catch (error) {
...
async function updateCrop(req, res) {
  try {
    ...
    console.log(
      `[updateCrop] Updating crop ${cropId} for farmerId: ${farmerId}`
    );
    ...
    if (cropId.includes("_")) {
      ...
      console.log(
        `[updateCrop] Found underscore format, landIndex: ${landIndex}`
      );
    } else {
      ...
      console.log(
        `[updateCrop] Found simple number format, converted ${cropId} to landIndex: ${landIndex}`
      );
    }
    ...
    if (
      ...
      console.log(
        `[updateCrop] Crop not found. landIndex: ${landIndex}, landsCount: ${
          lands.length
        }, hasCrop: ${lands[landIndex]?.crop ? "yes" : "no"}`
      );
    ...
    console.log(
      `🔧 DEBUG: Inventory check - Old: ${oldStatus}(${oldPercentage}%) → New: ${newStatus}(${newPercentage}%)`
    );
    ...
    if (newStatus === "Harvesting" && oldStatus !== "Harvesting") {
      ...
      console.log(
        `🔧 DEBUG: Adding to inventory - status changed to Harvesting`
      );
      ...
      console.log(`🔧 DEBUG: Removing from inventory - field clearing`);
      ...
      console.log(
        `🔧 DEBUG: Removing from inventory - harvesting 100% complete`
      );
      await manageInventory(farmerId, updatedCropData, landIndex, "remove");
    }
...
async function deleteCrop(req, res) {
  try {
    ...
    console.log(
      `[deleteCrop] Deleting crop ${cropId} for farmerId: ${farmerId}`
    );
    ...
    if (
      ...
      console.log(
        `[deleteCrop] Crop not found. landIndex: ${landIndex}, landsCount: ${lands.length}`
      );
      return res.status(404).send({ error: "Crop not found" });
    }
    ...
    console.log(
      `[deleteCrop] Successfully deleted crop from land ${landIndex}`
    );
...
async function getFrontendShipments(req, res) {
  try {
    const farmerId = req.user.uid;
    console.log(
      `[getFrontendShipments] Fetching shipments for farmerId: ${farmerId}`
    );
    ...
    console.log(
      `[getFrontendShipments] Returning ${approvedShipments.length} approved, ${shipmentRequests.length} requests`
    );
...
async function getFrontendItems(req, res) {
  try {
    ...
    console.log(
      `[getFrontendItems] Returning ${items.length} transformed items`
    );
    res.json(items);
  } catch (err) {
...
async function createShipment(req, res) {
  try {
    ...
    const docRef = await db.collection("farmer_shipments").add(shipmentData);
...
async function getDashboardData(req, res) {
  try {
    const farmerId = req.user.uid;
    console.log(
      `[getDashboardData] Fetching dashboard data for farmerId: ${farmerId}`
    );
...
async function getFrontendDashboard(req, res) {
  try {
    const farmerId = req.user.uid;
    console.log(
      `[getFrontendDashboard] Fetching frontend dashboard data for farmerId: ${farmerId}`
    );
...
async function approveShipmentRequest(req, res) {
  try {
    ...
    console.log(
      `[approveShipmentRequest] Approving shipment ${requestId} for farmerId: ${farmerId}`
    );

    // Get the shipment document
    const shipmentRef = db.collection("farmer_shipments").doc(requestId);
    ...
    console.log(
      `[approveShipmentRequest] Successfully approved shipment ${requestId}`
    );
    res.json({
...
async function submitShipmentReport(req, res) {
  try {
    ...
    const { containers, totalWeight, notes, status, qrcode } = req.body;

    console.log(
      `[submitShipmentReport] Submitting report for shipment ${shipmentId} by farmer ${farmerId}`
    );

    // Log QR code presence for debugging
    console.log(`[submitShipmentReport] QR code provided: ${!!qrcode}`);
    if (qrcode) {
      console.log(`[submitShipmentReport] QR code preview: ${qrcode.substring(0, 50)}...`);
    }
    ...
    // Use status from request body or default to "ready_for_pickup"
    const shipmentStatus = status || "ready_for_pickup";

    // Update shipment with report data including QR code
    const reportData = {
    ...
    // Add QR code to shipment if provided
    if (qrcode) {
      reportData.qrcode = qrcode;
      console.log(`[submitShipmentReport] Adding QR code to shipment ${shipmentId}`);
    }

    await db.collection("farmer_shipments").doc(shipmentId).update(reportData);

    console.log(
      `[submitShipmentReport] Successfully updated shipment ${shipmentId} with report data and status: ${shipmentStatus}`
    );
