...
async function getAvailableShifts(req, res) {
  try {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      for (const [shiftName, times] of Object.entries(shiftsConfig)) {
        ...
        if (shiftName !== "night") {
          ...
          shiftStart = shiftStart.set({ year: day.year, month: day.month, day: day.day });
          shiftEnd = shiftEnd.plus({ days: 1 }).set({ 
            year: day.plus({ days:1 }).year, 
            month: day.plus({ days:1 }).month, 
            day: day.plus({ days:1 }).day 
          });
        }
        ...
        if (shiftStart > now) {
          ...
          if (stockDoc.exists) {
            const deliveryTime =
              shiftEnd.minus({ hours: 1 }).toFormat("HH:mm") + "-" + shiftEnd.toFormat("HH:mm");
    ...
    // 🔘 Always append your fixed stockId here
    const fixedStockId = "LC-1_AS_2025_08_08_morning";
    const fixedDoc = await db.collection("availableMarketStock").doc(fixedStockId).get();
    if (fixedDoc.exists) {
      // Hardcode the label (no need for shiftEnd/day)
      availableShifts.push({
        id: fixedStockId,
        label: "Test Shift (Delivery: 06:00-07:00)" // you can put any label you want
      });
    }
