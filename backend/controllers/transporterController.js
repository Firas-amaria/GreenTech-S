...
async function verifyShipmentQRCodes(req, res) {
  try {
    ...
    console.log(`[verifyShipmentQRCodes] Looking in collection: ${collectionName}`);
    ...
    if (!shipmentDoc.exists && !shipmentType) {
      console.log(`[verifyShipmentQRCodes] Not found in ${collectionName}, trying customer_shipments`);
      collectionName = "customer_shipments";
    ...
    if (!shipmentDoc.exists) {
      console.log(`[verifyShipmentQRCodes] Shipment ${shipmentId} not found in any collection`);
    ...
    const shipmentData = shipmentDoc.data();
    console.log(`[verifyShipmentQRCodes] Found shipment in ${collectionName}, status: ${shipmentData.status}`);
