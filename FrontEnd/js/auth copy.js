...
async function saveTokenToFile(token, role) {
  try {
    // Create a textarea element to copy token to clipboard
    ...
    alert(
      `✅ ${role.toUpperCase()} TOKEN copied to clipboard!\n\nToken: ${token}\n\nPaste this into your .env file as:\n${role.toUpperCase()}_TOKEN=${token}`
    );
  } catch (error) {
    console.error("Error copying token:", error);

    // Final fallback - just show the token
    prompt(`Copy this ${role.toUpperCase()} TOKEN:`, token);
  }
}
