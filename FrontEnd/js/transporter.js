...
function renderAvailableShipments() {
  ...
  // Filter shipments to only show those with status "available_shipment"
      ...
      shipment.containers && shipment.containers.length > 0
        ? shipment.containers
            .map(
              (container) => `
          <tr>
            <td>${container.item}</td>
            <td>Code: ${container.code}</td>
            <td>${container.weightKg}kg</td>
            <td>Size: ${container.size}, Brix: ${container.brix}, Acidity: ${container.acidity}</td>
          </tr>
        `
            )
            .join("")
...
async function getShipmentDetails(shipmentId, shipmentType) {
  try {
    // Transporters should always use the transporter endpoint to get shipment details
...
function updateNotificationContent(shipment) {
  ...
  if (shipmentDistance) {
    ...
    const distance = shipment.distance || calculateDistance(shipment) || 'Unknown';
    shipmentDistance.textContent = typeof distance === 'number' ? `${distance} km` : distance;
  }
}
