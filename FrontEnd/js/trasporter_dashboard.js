  ...
  try {
    ...
    if (driversResponse.success) {
      ...
      dashboardState.drivers = driversResponse.drivers.map((driver) => {
        ...
        const driverShipments = allShipments.filter(
          (shipment) =>
        ...
        if (driverShipments.length > 0) {
          ...
          if (activeShipment) {
            const shipmentType =
              activeShipment.type === "farmer_shipment"
                ? "Farmer Pickup"
                : "Customer Delivery";
          ...
          if (
            todaySchedule &&
            isDriverScheduledNow(todaySchedule, currentTime)
          ) {
            processedDriver.status = "available";
            processedDriver.currentAssignment = "On Schedule - Available";
          } else {
            processedDriver.status =
              driver.availability || driver.status || "off_duty";
...
function renderShipmentsTable() {
  ...
  tbody.innerHTML = dashboardState.filteredData.shipments
    .map((shipment) => {
      ...
      // Determine shipment type display and CSS class
      let typeDisplay, typeClass;
      if (shipment.type === "farmer_shipment") {
        typeDisplay = "Farm → Logistics";
        typeClass = "type-farmer";
      } else if (shipment.type === "customer_shipment") {
        typeDisplay = "Logistics → Customer";
        typeClass = "type-customer";
      } else if (shipment.type === "order") {
        typeDisplay = "Order → Customer";
      ...
      // Truncate Shipment ID to max 20 characters
      const truncatedId = shipment.id && shipment.id.length > 20 ? shipment.id.slice(0, 20) + '…' : shipment.id;
      return `
      <tr>
        <td style="max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${truncatedId}</td>
        <td>
          <span class="shipment-type-badge ${typeClass}">
            ${typeDisplay}
          </span>
        </td>
...
function getShipmentStatus(shipment) {
  if (shipment.type === "farmer_shipment") {
    ...
    // For orders, use the status field directly
...
function getShipmentDestination(shipment) {
  if (shipment.type === "farmer_shipment") {
    ...
    // For orders, use deliveryAddress
    if (shipment.deliveryAddress) {
      return `${shipment.deliveryAddress.street}, ${shipment.deliveryAddress.city}`;
    }
    return shipment.destination || "Customer Location";
  } else {
    return (
...
function getShipmentPickupTime(shipment) {
  ...
  if (shipment.type === "order") {
    // For orders, use deliveryDate
    pickupTime = shipment.deliveryDate || shipment.pickup_time || shipment.scheduledPickup || shipment.createdAt;
  } else {
    pickupTime = shipment.pickup_time || shipment.scheduledPickup || shipment.createdAt;
  }
